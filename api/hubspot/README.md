...
client, _ := NewClient(hubspot.SetAPIKey("YOUR_API_KEY"))
```
...
client, _ := NewClient(hubspot.SetOAuth(&OAuthConfig{
...
client, _ := NewClient(hubspot.SetPrivateAppToken("YOUR_ACCESS_TOKEN"))
```
...
res, _ := client.CRM.Contact.Get("yourContactID", &Contact{}, nil)

// Type assertion to convert `interface` to `hubspot.Contact`.
contact, ok := res.Properties.(*Contact)
if !ok {
...
// Call associate api.
client.CRM.Contact.AssociateAnotherObj("yourContactID", &AssociationConfig{
    ToObject:   ObjectTypeDeal,
    ToObjectID: "yourDealID",
    Type:       AssociationTypeContactToDeal,
})
```
...
res, _ := client.CRM.Deal.Get("yourDealID", &CustomDeal{}, &RequestQueryOption{
